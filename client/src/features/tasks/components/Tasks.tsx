import { Plus, CheckCircle2, Circle, Trash2, Edit2, ListTodo, CheckSquare, Settings, HelpCircle } from "lucide-react";
...
import { formatDueDate, statusColor } from "@/features/tasks/utils";
...
export const Tasks = (): JSX.Element => {
  ...
  const setStatus = (task: Todo, status: TodoStatus) => {
                      ...
                      <div className="flex items-center gap-3 mt-1 flex-wrap">
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <button
                              type="button"
                              className={`font-['Montserrat',sans-serif] text-xs px-2 py-0.5 rounded ${statusColor(task.status).badge}`}
                              data-testid={`dropdown-status-${task.id}`}
                            >
                              {task.status}
                            </button>
                          </DropdownMenuTrigger>
                          <DropdownMenuContent align="start">
                            {(["pending", "inprogress", "completed", "cancelled"] as TodoStatus[]).map(
                              (status) => (
                                <DropdownMenuItem
                                  key={status}
                                  onSelect={() => setStatus(task, status)}
                                  className="flex items-center gap-2"
                                  data-testid={`dropdown-status-item-${task.id}-${status}`}
                                >
                                  <span
                                    className={`inline-block h-2 w-2 rounded-full ${statusColor(status).text.replace(
                                      "text-",
                                      "bg-",
                                    )}`}
                                  />
                                  <span className={statusColor(status).text}>{status}</span>
                                </DropdownMenuItem>
                              ),
                            )}
                          </DropdownMenuContent>
